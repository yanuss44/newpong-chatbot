...
app.post('/api/chat', async (req, res) => {
    ...
    const tryStreamGenerate = async () => {
        try {
            ...
            3. 추가 점검: 사용자가 "추가 점검"이나 "더 있나요?"라고 물을 때, 과거 대화 맥락을 확인하여 이미 제시한 방법 외에 더 이상 새로운 점검 사항이 없다면 아래와 같이 "no_more_checks"를 true로 반환하라. 절대 이전과 똑같은 내용을 반복하지 말 것.
            ...
            [해결책이 더 이상 없을 때의 JSON 구조]:
            {
              ...
              "symptom": "증상 요약",
              "cause": "추정 원인(최종)",
              "message": "추가적인 점검사항이 확인되지 않습니다. 제조사에서 면밀히 검토할수 있도록 C/S접수를 진행해 주세요",
              "steps": []
            }
